...
\textbf{Scenario 3: Categorical Dependent Variable and one Categorical Independent Variable}
...
The Shiny App that we'll be using lives in two places online. One instance of it \href{https://openscience.ok.ubc.ca/shiny/BIOL-116/}{can be found at this link} which is hosted on UBC servers. The other \href{https://ubco-biology.shinyapps.io/biol-116/}{can be found at this link} and is hosted on US servers. They are both identical in terms of what they allow you to do with your data.

\hypertarget{so-whats-the-difference}{%
\subsection*{So, what's the difference?}\label{so-whats-the-difference}}
\addcontentsline{toc}{subsection}{So, what's the difference?}

The instance on UBC servers cannot handle nearly as much user traffic as the one hosted in the US. And so it occasionally crashes when a lot of students are tyring to access it simultaneously. If you notice it's down, please let your TA know so we can reboot it!

We provide the UBC based option to allow students to access the app without having to worry about data sovereignty; when using this app, everything you share is kept within Canada. For lab 7, where you're only using pre-loaded data sets, this isn't a concern as you're not actively sharing any non-anonymous data with the server. However, for lab 9 when you'll be using your own data it's good to think about the nature of the data that you're working with and if there is any aspect of it that you might be concerned being on US servers. This includes not just the data you collected, but any associated meta data, such as a name or other identifier in the file name you upload.

Likely, there is nothing to be concerned about with this particular project, but it's important to have the choice! And it's important to always consider where one's research data - and personal data - are being transmitted and the implications this might have on who has access to that data. In fact, this is a key consideration when we talk about Open Science and the fact that it is not always our decision to choose what does and what does not get shared and along what channels or platforms it gets shared through.

Ok, and now in then next couple of pages you'll find a descriptive overview of what you'll encounter in the two panes of the Shiny application.
...
\textbf{Getting to Know Your Data}
...
\textbf{Additional Software Options}
...
\midrule
\endhead
Exceeds Expectations10 pts & Title is clear, concise and accurate representation of the experiment \\
Meets Expectations7.5 pts & Title is a mostly clear, concise and accurate representation of the experiment. Might be misplaced. \\
Below Expectations5 pts & Title is no clear or partially clear, OR is a poor representation of the experiment. \\
Not Acceptable2.5 pts & Title is not complete, AND is poorly written. \\
Not Present0 pts & Not present. \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Exceeds Expectations24 pts & Procedure is specific, address the question or problem, defines data collection clearly and specifies an appropriate control and independent and dependent variable(s). \\
Meets Expectations18 pts & Procedure is somewhat specific, addresses the question or problem and defines data collection fairly clearly with the appropriate controls and variables explained. \\
Below Expectations12 pts & Procedure is not specific, OR does not address the question or problem, OR does not define data collection clearly or specify the control and dependent and independent variables. \\
Not Acceptable6 pts & Procedure is not specific, AND does not address the question or problem, AND does not define data collection clearly or specify the control and dependent and independent variables. \\
Not Present0 pts & Procedure is no present. \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Exceeds Expectations12 pts & Relevant biology background is included, properly explained \& integrated with key findings. No factual errors. \\
Meets Expectations9 pts & Relevant background information is mostly present, AND is mostly correct (only one minor error), but not well integrated. \\
Below Expectations6 pts & Relevant background is confusing, OR has multiple minor errors AND is not well integrated. \\
Not Acceptable3 pts & Relevant background has major errors, but still present. \\
Not Present0 pts & Background is missing. \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Exceeds Expectations14 pts & Figure is included, describes key findings, includes controls, experiment is well-explained and integrates will with the summary of research findings. figure has a legend in authors own words AND is mentioned in text. \\
Meets Expectations10.5 pts & Figure describes key findings, includes controls, explanation of experiment is mostly good, integrates well with research findings. Author has described experiment in text AND figure legend is in own words. Only minor technical errors in explanation of figure. \\
Below Expectations7 pts & Not the best choice of figure (OR is incomplete but explanation is adequate), is partially explained, and/or has some incorrect interpretations. Author has either forgotten to include a figure legend (OR cut and paste figure legend from paper) OR not described figure in text (but legend does an adequate job). \\
Not Acceptable3.5 pts & Figure is present but is a poor choice OR is present but not explained in text/figure legend OR figure is not included but is adequately explained. \\
Not Present0 pts & Figure is not present, and not mentioned. \\
\bottomrule
\end{longtable}

\hypertarget{references-citation}{%
\subsection*{References \& Citation}\label{references-citation}}
\addcontentsline{toc}{subsection}{References \& Citation}
...
\midrule
\endhead
Exceeds Expectations8 pts & APA format is used properly \& consistently throughout. Sources are referenced as required. No more than 2 missed in-text citations, and no mismatches between citations \& reference list. \\
Meets Expectations6 pts & APA format is mostly properly used, errors are minor AND consistent. Primary papers are cited at least twice where it is discussed, but could be cited more. Reference list is present, formatted consistently and no more than one mismatch. \\
Below Expectations4 pts & APA formatting is not properly used OR formatting is inconsistent. Primary papers are cited in-text, but less than once. Other sources are cited as required. Reference list is present, but has mismatches with in-text citations. \\
Not Acceptable2 pts & APA formatting is not properly used OR formatting is inconsistent. Either in-text citation OR reference list is missing. Primary paper(s) are cited in text once only (or listed in the reference list with no in-text citation). Multiple mismatches between in-text citations and reference list. \\
Not Present0 pts & No in-text citations AND no reference list. If sources other than Primary papers are deemed to have been used, or plagiarism is suspected, more severe penalties may apply. \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Exceeds Expectations5 pts & No more than 1 small, with no grammar errors and no awkward phrasing. \\
Meets Expectations4 pts & No more than 3 small typos, AND only 1 grammar error. No more than one awkward sentence. \\
Below Expecttaions3 pts & More than 3 spelling mistakes, more than 1 grammar error and one awkward sentence. \\
Not Acceptable2 pts & Spelling errors are changing meanings of words, OR grammar is poor. Phrasing is awkward in multiple parts, and comprehension is beginning to be compromised. \\
...
\midrule
\endhead
Exceeds Expectations5 pts & Clearly stated in one sentence and supported by previous knowledge or research. \\
Below Expectations3 pts & Is somewhat vague, unclear, or wordy, and is somewhat based on previous knowledge or research. \\
Not Acceptable1 pts & Is somewhat vague, unclear, or wordy, and is somewhat based on previous knowledge or research. \\
Not Present0 pts & Hypothesis is not present. \\
\bottomrule
\end{longtable}

\hypertarget{study-significance}{%
\subsection*{Study Significance}\label{study-significance}}
\addcontentsline{toc}{subsection}{Study Significance}
...
\midrule
\endhead
Exceeds Expectations5 pts & The significance of this study has on society and/or the academic community is clearly outlined. \\
Meets Expectations3 pts & Significance of study is stated. \\
Not Acceptable0 pts & The significance of this study is not reported \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Exceeds Expectations5 pts & Results are clearly indicated interpreted and their impact on the field has been provided \\
Below Expectations3 pts & Results are clearly indicated, however, either no interpretation was provided or the impact the results have on the field is not provided. \\
Not Acceptable2 pts & Results are clearly indicated; however, no interpretation was provided and the impact these results have on the field is absent. \\
Not Present0 pts & No results were provided. \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Full Marks5 pts & All five criteria are met. \\
Proficient4 pts & Four of the five criteria were met. \\
Satisfactory3 pts & Three of the five criteria were met. \\
Poor2 pts & Two of the criteria were met. \\
Unsatisfactory1 pts & Only one criterion was met. \\
No Marks0 pts & No criteria were met. \\
\bottomrule
\end{longtable}
